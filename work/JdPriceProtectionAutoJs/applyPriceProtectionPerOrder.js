...
function needToEnterOrder(lastOrderCreateTime) {
    ...
    if (!viewPager) {
        viewPager = className("androidx.viewpager.widget.ViewPager").scrollable(true).findOne(6000)
    ...
    if (viewPager) {
        viewPager.children().forEach(child => {
            ...
            let target = child.findOne(className("androidx.recyclerview.widget.RecyclerView").scrollable(true));
            if (target) {
                recyclerViewOrderList = target;
            }
        })
    }
    console.log("recyclerViewOrderList find by viewPager:", recyclerViewOrderList)
    ...
    // }
    if (!recyclerViewOrderList) {
        console.log("找不到订单列表RecyclerView,考虑当前显示的页面不是订单列表页面，因此退出申请！")
        toastLog("找不到订单列表RecyclerView,考虑当前显示的页面不是订单列表页面，因此退出申请！")
        res.exit = true;
        return res;
    }
    ...
    if (nextEnterOrder) {
        ...
        nextEnterOrder.children().forEach(child => {
            let target = child.findOne(className("android.widget.TextView").text("完成"));
            //获取这个订单中心区域的控件 用于计算点击的坐标位置
            orderCenterArea_Wz = child.find(id("com.jd.lib.ordercenter.feature:id/wz").className("android.widget.RelativeLayout"))
            if (target) {
...
function applyPriceProtectionPerOrder() {
    ...
    while (true) {
        ...
        if (deduceEnterRes.exit) {
            ...
            exit();
            return;
        } else if (needToEnterOrder().enterItem) {
        ...
        waitTime(1, "等待返回订单页面")
        swipe(600, 900, 600, 900 + scrollY, 1000)
        waitTime(2, "等待滚动结束")
    }
    exit();
...
function enterOrder() {
    ...
    let enterOrderClickRes = click(enterOrderClickPoint.x, enterOrderClickPoint.y);
    ...
    let recyclerView = id("com.jd.lib.ordercenter.feature:id/uu").className("androidx.recyclerview.widget.RecyclerView").findOne(6000)
    console.log("recyclerView by fullId:", recyclerView)
    if (!recyclerView) {
        recyclerView = id("uu").className("androidx.recyclerview.widget.RecyclerView").findOne(6000)
        console.log("recyclerView by id:", recyclerView)
        if (!recyclerView) {
            recyclerView = className("androidx.recyclerview.widget.RecyclerView").scrollable(true).depth(5).findOne(6000)
    ...
    if (recyclerView) {
        recyclerView.children().forEach(child => {
            var target = child.findOne(className("android.widget.TextView").text("去申请"));
            if (target) {
...
function getOrderCreateTime() {
    ...
    let allOrderInfoBtn = className("android.widget.TextView").clickable(true).text("全部订单信息").findOne(6000)
    if (allOrderInfoBtn) {
        allOrderInfoBtn.click();
        waitTime(1)

        //根据这个fullId找到多个TextView,订单信息和订单创建时间是这个fullId
        let orderCreateTimeTextView_all = id("com.jd.lib.ordercenter.feature:id/xq").className("android.widget.TextView").find();
        ...
        if (!orderCreateTimeTextView) {
            orderCreateTimeTextView = id("xq").className("android.widget.TextView").findOne(6000)

        }
        if (orderCreateTimeTextView) {
            console.log("orderCreateTimeTextView:", orderCreateTimeTextView)
            console.log("getOrderCreateTime订单创建时间:", orderCreateTimeTextView.text())
...
function getOrderCreateTimeAnotherWay() {
    ...
    let orderInfoLayout = id("com.jd.lib.ordercenter.feature:id/wf").findOne(6000);
    if (!orderInfoLayout) {
        orderInfoLayout = id("wf").findOne(6000);
        if (!orderInfoLayout) {
            orderInfoLayout = className("android.widget.LinearLayout").clickable(true).findOne(6000)
        }
    }
    let orderInfoExpandedBtn;
    if (orderInfoLayout) {
        let orderInfoTextView = orderInfoLayout.findOne(className("android.widget.TextView").textContains("订单信息"));
        let btn = orderInfoLayout.findOne(className("android.widget.TextView").clickable(true).text("展开"));
    ...
    if (orderCreateTimeTextView) {
        console.log("orderCreateTimeTextView:", orderCreateTimeTextView)
        console.log("getOrderCreateTimeAnotherWay 订单创建时间:", orderCreateTimeTextView.text())
        return orderCreateTimeTextView.text();
    }
...
function enterToApply() {
    ...
    if (!applyBtn) {
        ...
        if (!dataListView) {
            ...
            console.log('dataListView by className("android.widget.ListView"):', dataListView)
        }
        console.log("最后一步的申请页面的dataListView(其中包含了申请按钮):", dataListView)
        if (dataListView) {
            dataListView.children().forEach(child => {
                let target = child.findOne(className("android.view.View").text("申请"));
                if (target) {
                    applyBtn = target;
                    console.log('applyBtn by child.findOne(className("android.view.View").text("申请")); ', applyBtn)
                }
            });
        }
